...
describe('ApiService', () => {
  ...
  describe('When call sendGetRequest()', () => {
    it('should get first page of products when call sendGetRequest', done => {
      ...
      apiService.sendGetRequest().pipe(retry(3)).subscribe(res => {
    ...
    it('should try 3 times when call sendGetRequest before send an error msg', done => {
      expect.assertions(3);
      mockHttpClient.get = jest.fn().mockImplementation(() => asyncError(mockErrorResponse));
      const errorMsg = 'Error Code: 404\nMessage: Http failure response for http://localhost:3000/productss?_page=1&_limit=5: 404 undefined';
      apiService.handleError = jest.fn().mockImplementation(() => throwError(errorMsg));
      ...
      apiService.sendGetRequest().pipe(retry(3)).subscribe(res => {},
        error => {
          expect(apiService.handleError).toBeCalledTimes(4);
          expect(error).toEqual(errorMsg);
          expect(apiService.parseLinkHeader).toBeCalledTimes(0);
          done();
        }
      )
    });//VALIDAR RETRY.
  ...
  describe('When call sendGetRequestToUrl(url)', () => {
    ...
    it('should try 3 times when call sendGetRequestToUrl() before send an error msg', done => {
      expect.assertions(3);
      const errorMsg = 'Error Code: 404\nMessage: Http failure response for http://localhost:3000/productss?_page=1&_limit=5: 404 undefined';
      mockHttpClient.get = jest.fn().mockImplementation(() => asyncError(mockErrorResponse));
      apiService.handleError = jest.fn().mockImplementation(() => throwError(errorMsg));
      ...
      apiService.sendGetRequestToUrl("http://localhost:3000/productss?_page=2&_limit=5").pipe(retry(3)).subscribe(res => {},
        error => {
          expect(apiService.handleError).toBeCalledTimes(4);
          expect(error).toEqual(errorMsg);
          expect(apiService.parseLinkHeader).toBeCalledTimes(0);
